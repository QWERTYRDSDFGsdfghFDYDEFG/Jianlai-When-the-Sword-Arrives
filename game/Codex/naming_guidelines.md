...
图片代号补充：写作与新演出调用可以使用 [图片代号规范](image_code_guide.md) 中的稳定编号。`../image_catalog.json` 是代号映射的唯一数据源；[总表](image_code_catalog.md) 与 [看图速查页](image_code_gallery.html) 由它生成。本文“三处同名”继续约束原始资源定义，正式代号别名不受第 3.2 节“仅临时兼容”的限制。已有图片无需批量改名，换素材版本时更新映射即可。
