...
- 以旁白交代旧事与黄庭国变化。
- 裴钱优先用 `peiqian lookaround` 这类轻动作版，保留入城和雀跃感。
...
- 不切背景，直接在郡城主街上引入吴懿。
- 吴懿如有动作版，优先用 `wy invite`，做出主动相邀而非普通站谈。
...
- 裴钱优先用 `peiqian lookaround`，避免室内也站成标准 pose。
...
- 长讲理段时，优先单人中景。
- 陈平安优先用 `cpa pressure` 或 `cpa listen` 这种收着的动作版，不靠大幅手势。
- 若有配音，必须拆句。
- 结尾再把听众切回。
...
- 与掌柜对戏时，优先给陈平安一个“边看货边问”的动作版，而不是正面对话。
- 古董铺信息量大，背景一次到位后不再切。
...
- 顾璨居中最重要。
- 即使先用占位，也不要直接套 `gc standard` 做最终图，最好保留高位和压场关系。
- 如果还没有群像立绘，后景人物可以先不做。
...
- 一路沉默感主要靠顾璨说、陈平安不接。
- 顾璨优先用 `gc chase_talk`，陈平安优先用 `cpa walk_talk`，让“追着说”和“继续往前”同时成立。
...
- 妇人跪姿必须低位，才能成立压迫感。
- `assassin kneel` 不能只缩小比例，必须是专门的低位姿势。
...
- 问题抛出后，画面应更干净。
- 最好只剩陈平安、顾璨、小泥鳅。
- 陈平安优先用 `cpa pressure`，顾璨优先从 `gc chase_talk` 过渡到更乱的反应姿态。
...
- 即便切中心位，也优先保持 `cpa pressure` 的克制动作，不要变成发怒 pose。
...
- 若只有一套动作版，优先保留“并行向前”的版本，而不是正面并肩看镜头。
